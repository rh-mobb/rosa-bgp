#!/usr/bin/env bats
# Test connectivity from CUDN VM to NodePort service with externalTrafficPolicy=Cluster on different node
...
setup() {
    # Get NodePort number
    export NODEPORT=$(oc get svc hello-openshift-nodeport-diffnode -n cudn1 -o jsonpath='{.spec.ports[0].nodePort}' 2>/dev/null)
    if [ -z "$NODEPORT" ]; then
        skip "hello-openshift-nodeport-diffnode service not found. Run tests/setup/setup-vm-ssh.sh first."
    fi
    ...
    # Verify backing pod is on a different node than test-vm-a
    local pod_node_name=$(oc get pod -n cudn1 -l app=hello-openshift-nodeport-diffnode -o jsonpath='{.items[0].spec.nodeName}' 2>/dev/null)

    if [ "$pod_node_name" = "$VM_A_NODE" ]; then
        skip "Backing pod is on the same node as test-vm-a (pod: $pod_node_name, vm: $VM_A_NODE)"
    fi
}

# Test: test-vm-a can curl NodePort service on different node
