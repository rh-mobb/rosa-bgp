#!/usr/bin/env bats
# Test connectivity from CUDN VM to NodePort service with externalTrafficPolicy=Cluster on same node
...
setup() {
    # Get NodePort number
    export NODEPORT=$(oc get svc hello-openshift-nodeport-samenode -n cudn1 -o jsonpath='{.spec.ports[0].nodePort}' 2>/dev/null)
    if [ -z "$NODEPORT" ]; then
        skip "hello-openshift-nodeport-samenode service not found. Run tests/setup/setup-vm-ssh.sh first."
    fi
    ...
    local pod_node=$(oc get pod -n cudn1 -l app=hello-openshift-nodeport-samenode -o jsonpath='{.items[0].status.hostIP}' 2>/dev/null)
    local pod_node_name=$(oc get pod -n cudn1 -l app=hello-openshift-nodeport-samenode -o jsonpath='{.items[0].spec.nodeName}' 2>/dev/null)
...
# Test: test-vm-a can curl NodePort service on same node
