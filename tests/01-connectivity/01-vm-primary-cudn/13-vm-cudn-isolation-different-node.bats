#!/usr/bin/env bats
# Test CUDN isolation between VMs on different CUDNs on different nodes
...
setup() {
    ...
    export VM_B_DIFFERENTWORKER_AS_A_IP=$(get_vm_ip test-vm-b-differentworker-as-a cudn2 2>/dev/null)
    if [ -z "$VM_B_DIFFERENTWORKER_AS_A_IP" ]; then
        skip "test-vm-b-differentworker-as-a is not running. Run tests/setup/setup-vm-ssh.sh first."
    fi

    # Verify VMs are on different nodes
    local vm_a_node=$(oc get vmi test-vm-a -n cudn1 -o jsonpath='{.status.nodeName}' 2>/dev/null)
    local vm_b_differentworker_as_a_node=$(oc get vmi test-vm-b-differentworker-as-a -n cudn2 -o jsonpath='{.status.nodeName}' 2>/dev/null)

    if [ "$vm_a_node" = "$vm_b_differentworker_as_a_node" ]; then
        skip "test-vm-a and test-vm-b-differentworker-as-a are on the same node"
    fi
}
...
# Test: test-vm-a cannot ping test-vm-b-differentworker-as-a (different CUDN, different node)
@test "test-vm-a cannot ping test-vm-b-differentworker-as-a (CUDN isolation, different node)" {
    assert_vm_cannot_ping_vm test-vm-a cudn1 "$VM_B_DIFFERENTWORKER_AS_A_IP"
}

# Test: test-vm-a cannot curl test-vm-b-differentworker-as-a (different CUDN, different node)
@test "test-vm-a cannot curl test-vm-b-differentworker-as-a (CUDN isolation, different node)" {
    assert_vm_cannot_curl_vm test-vm-a cudn1 "$VM_B_DIFFERENTWORKER_AS_A_IP"
}
